...
- 일정 알림(앱 실행 중 배너)
  - 우측 상단 현재 시간 기준 10분 내 일정 알림(5분 주기 스캔)
...
- 문자열 검색은 `EF.Functions.Like` 기반으로 처리하고 패턴 이스케이프를 적용해 검색 정확성을 보완
...
- 검색어는 공백을 정리(Trim + 다중 공백 축약)한 뒤 쿼리에 반영
- SQL Server의 대소문자 비구분(CI) 콜레이션 환경에서 `LIKE` 검색을 사용해 `ToUpper()` 컬럼 변환 없이 비교

 → 대소문자/공백 입력 차이에 강한 검색 경험을 제공하고, 인덱스 활용에 유리한 쿼리 형태를 유지
